			...
			<?php 
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
						...
						$sql= "SELECT course_name FROM course_table WHERE course_date = '".$currentDate."' AND course_status = 1";
						...
						if(strtotime($currentDate) == strtotime(date("Y-m-d"))){
							...
							// Popover Content 
							$sql="SELECT course_name, course_time FROM course_table WHERE course_date = '".$currentDate."' AND course_status = 1 ORDER BY course_time asc";
								$result = mysqli_query($conn,$sql) ;
								$Ec=1; 
